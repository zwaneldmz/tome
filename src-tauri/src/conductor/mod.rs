...
//! hands over the workspace via 7 tools, tracks per-pane scrollback +
...
//!   mirroring `conductor.js`'s own `init(opts)` DI shape plus one net-new
//!   field this port needs that JS didn't (`stream_chat` — see that
//!   module's doc comment).
//! - [`tools`] — the 7 tool JSON schemas, `runTool` dispatch, and the two
