//! Airgap subsystem (Phase 3) — the pane-gapping state machine, repo-
...
//! crate (`ipc::airgap`, `ipc::pty`, `export.rs`, `schedule.rs`'s
//! `sha1_hex` use, …) keeps compiling unchanged.
//!
//! `ipc::airgap::create_gapped_pane_proxy`/`close_pane_and_proxy` are NOT
//! part of this move — they are `pub(crate)` functions defined in
//! `ipc/airgap.rs` itself (the Tauri-facing integrator this module's own
//! doc comment, ported below unchanged, calls "Task A4"), not in this
//! module, and stay exactly where they were.

pub use tome_flow::airgap::*;
