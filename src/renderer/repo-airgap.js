// Repo-committed air-gap allowlist (.tome/airgap.json): detection and consent.
// The file is UNTRUSTED — anyone who can commit to the repo can widen the
// egress of every air-gapped pane pointed at it — so nothing from it takes
...
// VERIFIED AND STORED in main (userData/airgap-repo-consents.json, 0600,
...
async function consent(root, hash) {
  try {
    const r = await tome.airgap.consentRepo(root, hash)
    if (!r.ok) {
    ...
    if (r.applied?.length)
      toast(`repo allowlist active: ${r.applied.length} hosts from .tome/airgap.json`, 'ok')
  } catch (err) {
...
function consentModal(root, hosts, rejected) {
  return new Promise((resolve) => {
    const m = modalShell(`⛨ ${root.split('/').pop() || root} asks to widen the air gap`, () =>
      resolve(false)
    )
    m.note(
      'This repo commits a .tome/airgap.json. Anyone who can commit to this repo can edit this list; it stays in force until the file changes.'
    )
...
async function checkFolder(root) {
  ...
  const r = await tome.airgap.readRepo(root)
  ...
  if (!(await consentModal(root, r.hosts, r.rejected))) {
    toast('repo air-gap allowlist ignored', 'err')
...
export async function checkRepoAirgap() {
  if (running) return
  running = true
  try {
    ...
    if (!String(err?.message).includes('Tome is locked')) console.warn('repo airgap check:', err)
  } finally {
    running = false
  }
}
